    ...
    <q-layout ref="layout" view="hHh LpR lFf" class="bg-grey-9">
      <q-header v-if="isVisibleToolbar">
        <q-toolbar class="header__main-toolbar bg-grey-9">
          ...
          />
          <q-toolbar-title
            :style="{ minWidth: $q.platform.is.mobile ? '60px' : '210px' }"
          >
            <img
              class="gt-sm cursor-pointer"
              src="toolbox50.png"
              alt="Toolbox"
              style="height: 30px"
              @click="goToMain"
            />
            <img
              class="lt-md cursor-pointer"
              src="toolbox_mobile.png"
              alt="Toolbox"
              style="height: 30px"
              @click="goToMain"
            />
          ...
          >
            <q-chip
              v-if="newNotificationCounter"
              color="red"
              size="xs"
              class="absolute-top-right q-ma-none text-white"
              >{{ newNotificationCounter }}</q-chip
            >
      ...
      <q-drawer
        side="left"
        v-model="sides.left"
        :content-class="{ 'bg-grey-7': true }"
        v-if="isVisibleToolbar && !dashMode"
      >
        <left-menu
          :config="config"
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      localeName: (state) =>
        ...
        state.sessionSettings.region &&
        state.sessionSettings.region.name,
      ...
      set(val) {
        if (val < 100) {
          ...
          this.$q.notify({
            ...
            timeout: 10000,
            message:
              "You are setting the row count to more than 2000. This can affect your browser performance. Do you want to continue?",
            position: "top-right",
            actions: [
              {
                label: "Agree",
  ...
  methods: {
    ...
    routeProcess(route) {
      ...
      if (route.query.token && !this.token) {
        ...
        Vue.connector.socket.off("connect", routeProcessIndex);
        this.$router
          .push({ name: "simpleLogin", params: { goto: route } })
          .catch((err) => err);
      } else {
  ...
  created() {
    ...
    if (sessionSettings.isNeedSelect !== undefined)
      this.isNeedSelect = sessionSettings.isNeedSelect;
    if (sessionSettings.isVisibleToolbar !== undefined)
      this.isVisibleToolbar = sessionSettings.isVisibleToolbar;
    if (sessionSettings.mode !== undefined)
      this.toolboxMode = sessionSettings.mode;
    if (!this.isInit) {
      this.connectFlag = true;
      this.connectionPreserveHandlerIndex = Vue.connector.socket.on(
        "connect",
        this.connectionPreserveHandler
      );
    }
    this.setToolboxSessionSettings({
      isNeedSelect: this.isNeedSelect,
      isVisibleToolbar: this.isVisibleToolbar,
      mode: this.toolboxMode,
    });
  },
  beforeDestroy() {
    this.connectionPreserveHandlerIndex !== undefined &&
      Vue.connector.socket.off("connect", this.connectionPreserveHandlerIndex);
  },
